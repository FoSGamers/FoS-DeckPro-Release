...
class ManaBoxEnhancerGUI:
    ...
    def setup_gui(self):
        ...
        # Add Generate Listing button below the preview
        ttk.Button(preview_frame, text="Generate Listing", command=self.show_listing_popup).pack(padx=10, pady=(0,10))
    ...
    def display_cards(self):
        ...
        # Ensure filtered_data is always set for pagination and other features
        self.filtered_data = filtered_data
            ...
            try:
                ...
                with open(self.existing_json_file, 'r', encoding='utf-8') as f:
                    ...
                    if self.existing_data:
                        ...
                        # --- Mismatch detection and auto-reset ---
                        card_keys = set(self.existing_data[0].keys())
                        config_ids = set(col['id'] for col in self.column_config['columns'])
                        # If there is no intersection or a big mismatch, reset
                        if not config_ids.intersection(card_keys) or len(config_ids.intersection(card_keys)) < min(len(card_keys), len(config_ids)) // 2:
                            logging.warning("Column config does not match card data. Resetting to default columns.")
                            self.column_config = {"columns": [col.copy() for col in DEFAULT_COLUMNS]}
                            self.save_column_config()
    ...
    def generate_listing(self, card):
        ...
        collector_number = card.get("Collector number", "")
        condition = card.get("Condition", "").replace("_", " ").title()
        foil = "Foil" if str(card.get("Foil", "")).lower() == "foil" else ""
        language = card.get("Language", "en").title()
        title_parts = [name]
        if set_name and collector_number:
            title_parts.append(f"({set_name} #{collector_number})")
        if condition:
            title_parts.append(f"- {condition}")
        if foil:
            title_parts.append(foil)
        if language and language.lower() != "En":
            title_parts.append(language)
        title = " ".join(title_parts).replace("  ", " ").strip()

        # Description (stop at Oracle Text)
        desc_lines = [
            f"Card: {name}",
            f"Set: {set_name} ({card.get('Set code', '')})",
            f"Collector Number: {collector_number}",
    ...
    def show_listing_popup(self):
        selection = self.card_display.selection()
        if not selection:
            messagebox.showerror("Error", "Please select a card to generate a listing.")
            return
        # Get the selected item's values
        item = self.card_display.item(selection[0])
        values = item['values']
        ...
        for c in data:
            if all(str(c.get(col["id"], "")) == str(val) for col, val in zip(self.column_config["columns"], values)):
                ...
                break
        if not card:
            messagebox.showerror("Error", "Could not find card data.")
            return
        title, description = self.generate_listing(card)
        # Popup window
        popup = tk.Toplevel(self.root)
        popup.title("Generated Listing")
        popup.geometry("600x400")
        # Title label
        ttk.Label(popup, text="Title:", font=("Arial", 10, "bold")).pack(anchor=tk.W, padx=10, pady=(10,0))
        title_entry = tk.Text(popup, height=2, wrap=tk.WORD)
        title_entry.insert(tk.END, title)
        title_entry.config(state=tk.DISABLED)
        title_entry.pack(fill=tk.X, padx=10, pady=(0,10))
        # Description label
        ttk.Label(popup, text="Description:", font=("Arial", 10, "bold")).pack(anchor=tk.W, padx=10)
        desc_text = tk.Text(popup, height=12, wrap=tk.WORD)
        ...
        def copy_to_clipboard():
            ...
            messagebox.showinfo("Copied", "Listing copied to clipboard!")
        ttk.Button(popup, text="Copy to Clipboard", command=copy_to_clipboard).pack(pady=5)
