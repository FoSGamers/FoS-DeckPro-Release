    ...
    def load_commands(self) -> None:
        ...
        try:
            ...
            if not os.path.exists(commands_path):
                ...
                self.save_commands()  # Save default commands
                return
            ...
            # Convert the commands array into our expected format
            for cmd in data.get('commands', []):
                if not isinstance(cmd, dict):
                    logger.warning("Invalid command format, skipping")
                    continue
                ...
                if not name:
                    logger.warning("Command missing name, skipping")
                    continue
                ...
                self.commands[category][name] = {
                    ...
                    'usage': cmd.get('usage', ''),
                    'permission_level': cmd.get('permission_level', 'basic'),
                    'enabled': cmd.get('enabled', True)
                }
                
            logger.info("Commands loaded successfully")
        except json.JSONDecodeError:
            logger.error("Invalid JSON in commands file")
