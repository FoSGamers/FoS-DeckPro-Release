...
DEFAULT_COLUMNS = [
    {"id": "Name", "display": "Name", "width": 150, "visible": True, "filter_type": "search"},
    {"id": "Set code", "display": "Set code", "width": 100, "visible": True, "filter_type": "dropdown"},
    {"id": "Set name", "display": "Set name", "width": 150, "visible": True, "filter_type": "dropdown"},
    {"id": "Collector number", "display": "Collector number", "width": 120, "visible": True, "filter_type": "search"},
    {"id": "Foil", "display": "Foil", "width": 80, "visible": True, "filter_type": "checkbox"},
    {"id": "Rarity", "display": "Rarity", "width": 100, "visible": True, "filter_type": "checkbox"},
    {"id": "Quantity", "display": "Quantity", "width": 80, "visible": True, "filter_type": "search"},
    {"id": "ManaBox ID", "display": "ManaBox ID", "width": 100, "visible": True, "filter_type": "search"},
    {"id": "Scryfall ID", "display": "Scryfall ID", "width": 200, "visible": True, "filter_type": "search"},
    {"id": "Purchase price", "display": "Purchase price", "width": 100, "visible": True, "filter_type": "search"},
    {"id": "Misprint", "display": "Misprint", "width": 80, "visible": True, "filter_type": "checkbox"},
    {"id": "Altered", "display": "Altered", "width": 80, "visible": True, "filter_type": "checkbox"},
    {"id": "Condition", "display": "Condition", "width": 100, "visible": True, "filter_type": "checkbox"},
    {"id": "Language", "display": "Language", "width": 80, "visible": True, "filter_type": "checkbox"},
    {"id": "Purchase price currency", "display": "Purchase price currency", "width": 150, "visible": True, "filter_type": "dropdown"},
    {"id": "type_line", "display": "Type", "width": 150, "visible": True, "filter_type": "dropdown"},
    {"id": "mana_cost", "display": "Mana Cost", "width": 100, "visible": True, "filter_type": "search"},
    {"id": "colors", "display": "Colors", "width": 100, "visible": True, "filter_type": "dropdown"},
    {"id": "color_identity", "display": "Color Identity", "width": 120, "visible": True, "filter_type": "dropdown"},
    {"id": "oracle_text", "display": "Oracle Text", "width": 200, "visible": True, "filter_type": "search"},
    {"id": "legal_commander", "display": "Commander Legal", "width": 120, "visible": True, "filter_type": "checkbox"},
    {"id": "legal_pauper", "display": "Pauper Legal", "width": 100, "visible": True, "filter_type": "checkbox"},
    {"id": "cmc", "display": "CMC", "width": 80, "visible": True, "filter_type": "search"},
    {"id": "image_url", "display": "Image URL", "width": 200, "visible": True, "filter_type": "search"},
    {"id": "Whatnot price", "display": "Whatnot Price", "width": 100, "visible": True, "filter_type": "search"}
]

CHECKBOX_FILTER_FIELDS = {
    "Foil", "Rarity", "Misprint", "Altered", "Condition", "Language", "legal_commander", "legal_pauper", "Set code", "Set name"
}
...
class ManaBoxEnhancerGUI:
    ...
    def setup_gui(self):
        ...
        # --- Card Display and Filter Row with Synchronized Horizontal Scroll ---
        ...
        # Filter row
        self.filter_row_frame = ttk.Frame(self.inner_frame)
        self.filter_row_frame.pack(fill=tk.X, pady=(0, 2))
        ...
        # Card Preview
        preview_frame = ttk.LabelFrame(display_frame, text="Card Preview")
    ...
    def apply_filters(self):
        """Apply all filters to the entire dataset and update the table."""
        data = self.merged_data if self.merged_data else (self.new_data if self.new_data else self.existing_data)
        if not data:
            return
    ...
    def display_cards(self):
        ...
        for card in display_data:
            # Text filter
            if filter_text:
                if not any(filter_text in str(value).lower() for value in card.values()):
                    continue
            ...
            for key, var in self.active_filters.items():
                if var.get():
                    ...
                    if str(card.get(column, "")).lower() != value.lower():
                        skip = True
                        break
            if skip:
                continue
    ...
    def update_card_display_columns(self, data=None):
        """Update the card display columns based on configuration"""
        if not data:
            data = self.merged_data if self.merged_data else (self.new_data if self.new_data else self.existing_data)
        if not data:
    ...
    def create_filter_row(self):
        ...
        for col in visible_cols:
            col_id = col["id"]
            frame = ttk.Frame(self.filter_row_frame)
            frame.pack(side=tk.LEFT, fill=tk.Y, padx=1, pady=1)
            if col_id in CHECKBOX_FILTER_FIELDS:
                btn = ttk.Button(
                    frame,
                    text="Filter...",
                ...
                self.filter_values[col_id] = set()
            else:
                var = tk.StringVar()
                entry = ttk.Entry(frame, textvariable=var, width=max(8, len(col["display"])))
                entry.pack(fill=tk.X)
                entry.bind("<KeyRelease>", lambda e, c=col_id, v=var: self.on_text_filter_change(c, v))
                self.filter_widgets[col_id] = entry
                self.filter_values[col_id] = ""
    ...
    def _sync_canvas_width(self, event=None):
        ...
        # Set canvas width to match the Treeview's width
