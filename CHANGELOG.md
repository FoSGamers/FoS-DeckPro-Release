...
### Changed
- Backend trial logic is now 100% robust: it appends any missing trial columns (`machine_id`, `trial_start_1`, etc.) to the end of the sheet header, preserving all existing columns and data. No columns are ever overwritten or deleted. All trial logic now works regardless of the sheet's initial structure. Code comments and docstrings updated for clarity and maintainability.

### Fixed
- Trial logic header update now uses `worksheet.update('1:1', [new_header])` instead of delete/insert for 100% reliable header updates. This guarantees trial columns are always present and visible, regardless of sheet structure. Added print logging for header and trial logic to aid debugging. This change was made because Google Sheets API and gspread can silently fail to update the header with delete/insert, especially on large or filtered sheets.
...
### Changed
- Updated `.gitignore`, `.gcloudignore`, and `cloud_functions/license_check/README.md` to ensure `service_account.json` is always included in Cloud Function deployments, never committed to public repos, and fully documented for gold-standard security and workflow compliance.
