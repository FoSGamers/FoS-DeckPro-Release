...
def fuzzy_name_match(name, candidates, threshold=0.85):
    """Return best fuzzy match from candidates if above threshold, else None."""
    name = str(name).strip().lower()
    best = difflib.get_close_matches(name, [str(c).strip().lower() for c in candidates], n=1, cutoff=threshold)
